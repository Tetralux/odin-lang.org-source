...
`_` means "I don't need this."

You can assign to it normally:
```odin
_ = f();
_ = g();
_ = h();
...
In optimized programs, the value of unnamed variables like this will not be stored.

Another way it can be used is as a field name in a struct, although it's fairly unusual to use it this way:
```odin
Stuff :: struct {
    str: string,
    _: u64,
